...
const handlers = [
    // 使用docxjs支持，目前效果最好的渲染器
    {
        ...
        handler: async (buffer, target) => {
            const docxOptions = (<any>Object).assign(defaultOptions, {
                ...
                ignoreWidth: false,
                renderFooters: false, //disables rendering width of page
    ...
    // 错误处理
    {
        ...
        handler: async (buffer, target, type) => {
            target.innerHTML = `<div style="text-align: center; margin-top: 80px;color: #F7F7F7">不支持.${type}格式的文件预览，请下载后预览或转换为支持的格式</div>
<div style="text-align: center;color: #F7F7F7">支持docx, xlsx, pdf, 以及纯文本格式和各种图片格式的文件预览</div>`;
            return VueWrapper(target);
        },
    },
];
