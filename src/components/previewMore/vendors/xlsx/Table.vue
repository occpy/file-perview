  ...
  <div >
    ...
    <div class="btn-group" >
      <button
          ...
          :key="sheet.id"
          :class="sheetIndex === sheet.id ? 'primary' : ''"
          style="padding: 0 10px;height: 24px"
          :type="sheetIndex === sheet.id ? 'primary' : 'default'"
          @click="handleSheet(sheet.id)"
      >
       <div :title="sheet.name" class="hiddenMemo ">
         {{ sheet.name}}
       </div>
      </button>
    </div>
  ...
  computed: {
    hotSettings() {
      return {
        ...
        colHeaders: true,
        rowHeaders: true,
        height: localStorage.getItem("componentsHeightIP") - 47 + 'px',
        stretchH: true,
        // contextMenu: true,
        // manualRowMove: true,
        // 关闭外部点击取消选中时间的行为
